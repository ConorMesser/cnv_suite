...
class CNV_Profile:
    ...
    def generate_coverage(self, purity, cov_binned, x_coverage=None, sigma=None):
        ...
        if not sigma:
            sigma = 1
        x_coverage_df = pd.read_csv(cov_binned, sep='\t', names=['chrom', 'start', 'end', 'coverage', 'coverage2'],
                                    dtype={'chrom': str})
        ...
        return x_coverage_df[['chrom', 'start', 'end', 'cov_adjust', 'coverage', 'coverage2', 'ploidy']]
    ...
    def save_coverage_file(self, filename, purity, cov_binned_file, x_coverage=None, sigma=None):
        ...
        cov_df.replace(columns={'chrom': 'chr', 'cov_adjust': 'covcorr', 'coverage': 'covraw'}).to_csv(filename, sep='\t', index=False)  # todo is this what we want (column headings)?

    def generate_snvs(self, vcf, bed, purity):
        snv_df = pd.read_csv(vcf, sep='\t', comment='#')
        bed_df = pd.read_csv(bed, sep='\t', header=['CHROM', 'POS', 'DEPTH'])
        ...
        snv_df['maternal_prop'] = snv_df.apply(
            lambda x: (sorted(self.cnv_trees[x['CHROM']][1][x['POS']])[0].data['cn_change'] - 1) *
                      purity + (1 - purity) / x['total_ploidy'], axis=1)

        snv_df['paternal_prop'] = snv_df.apply(
            lambda x: (sorted(self.cnv_trees[x['CHROM']][0][x['POS']])[0].data['cn_change'] - 1) *
                      purity + (1 - purity) / x['total_ploidy'], axis=1)

        snv_df['maternal_present'] = snv_df['NA12878'].apply(lambda x: x[0] == '1')
        snv_df['paternal_present'] = snv_df['NA12878'].apply(lambda x: x[2] == '1')

        snv_df['final_depth'] = np.floor(snv_df['DEPTH'] * snv_df['ploidy'] / 2)
        ...
        snv_df['alt_count'] = snv_df.apply(
            lambda x: get_alt_count(x['maternal_prop'], x['paternal_prop'], x['maternal_present'],
                                    x['paternal_present'], x['final_depth'],
                                    correct_phase_interval_trees[x['CHROM']][x['POS']].pop().data), axis=1)
        snv_df['ref_count'] = snv_df.apply(lambda x: x['final_depth'] - x['alt_count'])

        return snv_df, correct_phase_interval_trees

    def save_hets_file(self, filename, vcf, bed, purity):
        vcf_df = self.generate_snvs(vcf, bed, purity)
        vcf_df.replace(columns={'CHROM': 'CONTIG', 'POS': 'POSITION',
...
def get_alt_count(m_prop, p_prop, m_present, p_present, coverage, correct_phase):
    ...
    if m_present and p_present:
        return coverage
    elif ~m_present and ~p_present:
        return 0
    elif m_present:
        alt_count = np.random.binomial(coverage, coverage * m_prop)
    else:
        alt_count = np.random.binomial(coverage, coverage * p_prop)
    return alt_count
...
def get_average_ploidy(tree, start, end, purity):
    ...
    def single_allele_ploidy(allele):
        ...
        interval_totals = [(min(i.end, end) - max(i.begin, start)) * (i.data['hscr.a2'] + i.data['hscr.a1']) for i in  # todo
                           intervals]
...
class Phylogeny:
    ...
    def get_lineage(self, subclone_num):
        node = subclone_num
        cluster_list = []
